...
function Message({ message }) {
  ...
  useEffect(() => {
    // Only highlight inside this message (not whole page)
    ...
    <div>
      ...
      {message.role === 'user' ? (
        ...
        <div className='flex justify-start my-4 w-full'>
          <div
            ref={messageRef}
            className='inline-flex flex-col gap-2 p-2 px-4 max-w-2xl bg-primary/20 dark:bg-[#57317C]/30 border border-[#80609F]/30 rounded-md'
          >
            {message.isImage ? (
              <img
                src={message.content}
